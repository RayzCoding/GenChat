...
import { streamChat, stopAgent } from '../services/agentApi'
...
export function useChatStream({ conversationId, onSessionUpdated }: UseChatStreamOptions) {
  ...
  const sendMessage = useCallback(
    async (question: string, t: (key: string) => string) => {
      ...
      try {
        await streamChat(conversationId, trimmed, controller.signal, (line) => {
          const chunk = parseAgentChunk(line)
          if (!chunk) return

          setTurns((prev) =>
            prev.map((turn) =>
              turn.id === assistantId ? applyChunk(turn, chunk) : turn,
            ),
          )
        })
